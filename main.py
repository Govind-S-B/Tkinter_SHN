...
mood = {"happy": "green", "confused": "grey",
        "sad": "blue", "star": "yellow", "angry": "red"}
...
# configured for 1500x1500 seed
crop_img = seed_img.crop((250, 250, 1250, 1250))
...
def move():
    ...
    canvas.create_image(0, 0, image=final_frame, anchor="nw")
    canvas.create_text(505, 150, text="How was your day ?",
                       font=qstn_font, fill="white")
    ...
    if happy_state == 0:
        ...
        happy_button = canvas.create_image(
            left_mar, 225, image=happy_emoji_big)
    ...
    if star_state == 0:
        ...
        star_button = canvas.create_image(
            left_mar+75, 225, image=star_emoji_big)
    ...
    if sad_state == 0:
        ...
        sad_button = canvas.create_image(
            left_mar+150, 225, image=sad_emoji_big)

    canvas.tag_bind(sad_button, "<Button-1>", sad_button_clicked)

    if confused_state == 0:
        confused_button = canvas.create_image(
            left_mar+225, 225, image=confused_emoji)
    else:
        confused_button = canvas.create_image(
            left_mar+225, 225, image=confused_emoji_big)

    canvas.tag_bind(confused_button, "<Button-1>", confused_button_clicked)

    if angry_state == 0:
        angry_button = canvas.create_image(
            left_mar+300, 225, image=angry_emoji)
    else:
        angry_button = canvas.create_image(
            left_mar+300, 225, image=angry_emoji_big)
...
e1 = Entry(canvas, background="white", borderwidth=3,
           textvariable=string, width=20, bd=0, font="havletica 12")
...
style.map('TButton', foreground=[
          ('active', '!disabled', 'green')], background=[('active', 'black')])


btn = Button(root, text='Submit', width=10, height=1, font="havletica 11",
             background="#255EC9", foreground="white", command=submit)
